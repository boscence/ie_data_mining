...
```{r}
bb = df[,grepl( "^p.*Slice$",names(df))]


cc = as.numeric(bb$p2.1.2.it1.Slice)
sum(cc)
dd = (cc - bb$p2.1.2.it1.Slice)
cbind(cc,bb$p2.1.2.it1.Slice,dd)
...
rowsum(as.numeric(bb))

bb$sum = colSums()

bb$sum = xx
    as.character(as.numeric(i)))}

tail(bb)
bb = numeric(bb)

names(df[ , grepl( "^p.*Slice$" , names( df ) ) ])
```
